...
- **강수확률**: 시간별 예보 기준 max. 이미 지나간 새벽 강수예보는 제외 (모순 메시지 방지)
...
- **라이트 테마 카드뉴스**: 연회색 배경 + 흰색 카드 + 부드러운 그림자. 도시별로 카드 한 장.
- **슈퍼샘플링(SS=2)**: 전체를 2배 해상도(1440px)로 그린 뒤 `LANCZOS`로 720px 다운스케일 → 안티앨리어싱. PIL은 기본적으로 안티앨리어싱을 하지 않아 그냥 그리면 텍스트·아이콘·곡선에 계단 현상이 생긴다.
- **`_ScaledDraw` 프록시**: 드로잉 코드는 모두 논리 좌표(1배)로 작성하고, 프록시가 좌표·선두께·반지름을 자동으로 SS배 확대해 실제 캔버스에 그린다. 덕분에 레이아웃 로직은 배율을 신경 쓸 필요가 없다.
- **곡선 차트**: 시간별 기온을 Catmull-Rom 스플라인(`_smooth`)으로 부드러운 곡선으로 그리고, 곡선 아래는 마스크 합성으로 세로 그라데이션(`_gradient_area`)을 채운다. 곡선·마커 색은 기온에 따라 파랑↔주황으로 보간(`_temp_color`).
- **그림자**: RGBA 레이어에 둥근 사각형을 그린 뒤 `GaussianBlur` → `alpha_composite`로 카드 아래 부드러운 그림자(`_soft_shadow`).
...
- **라이트 테마**, 도시별 흰색 카드 + 부드러운 그림자, 시간별 곡선 차트, 주간 미니 카드 포함
- **시간별 곡선 차트**: Catmull-Rom 스플라인 + 영역 그라데이션, 기온에 따라 파랑↔주황 보간
- **2배 슈퍼샘플링** 렌더 후 다운스케일 → 텍스트·아이콘·곡선이 또렷 (계단 현상 제거)
- **벡터 날씨 아이콘**: 해/구름/구름조금/비/눈/천둥을 코드로 분류해 직접 그림 (이모지 폰트 불필요)
- 통계 4분할(세로 구분선)·알약형 알림 배지로 정돈된 카드뉴스 스타일
